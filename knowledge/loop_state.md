...
- 21:45 SCORING FIX (user flag): full-pipeline fps was WRONG (dominated by client-side VAE). Profile: VAE conv ~82% GPU (client), DiT latent-gen ~18% (server). cuDNN-9.23 win was CLIENT (VAE), ~0 server value. CORRECT score = DiT-kernel time excl conv. Server latent-gen ~36ms/frame GPU (~20fps-ish) ALREADY. Next: precise latent-only baseline, then optimize DiT (FP8 GEMM) scored on DiT-kernel time.
- 22:18 SERVER AT TARGET: latent-gen 25.71 fps (3-step, 2p, warm, decode excluded). Target 20-30 MET. The 1.96fps panic was client VAE. Kernel work now = margin/scaling, not blocker. Open: validate 3-step quality.
