...
Q: keep optimizing in JAX (KV Craft's impl) or reimplement the serving DiT in PyTorch/CUDA?
...
- Port is BOUNDED: DiT is 1.5B, Wan2.1 architecture has reference PyTorch. Reimplement DiT forward
  + 3/4-step sampler + load JAX weights. KV Craft-JAX -> reference/weights/validation only.
...
## Plan if approved
- Short-term: keep JAX-KV Craft to PROVE levers fast (no reimpl).
